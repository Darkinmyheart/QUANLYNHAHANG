        ...
        try {
            if (txttennhanvien.getText().equals("") || txtsdt.getText().equals("")) {
                ...
                NhanVien newNhanVien = new NhanVien(
                        ...
                        txtNgaysinh.getDate(),
                        cboGt.getSelectedItem().toString(),
                        txtdiachi.getText(),
                        txtsdt.getText(),
                        txtEmail.getText(),
                        cboChucVu.getSelectedIndex() == 0 ? "Quản Lý" : "Nhân viên",
                        BigDecimal.valueOf(Double.parseDouble(txtLuong.getText())),
                        null,
                        null
                );
                new NhanVienDao().InsertSQL(newNhanVien);
                TaiKhoan newTaiKhoan = new TaiKhoan();
                newTaiKhoan.setIdNhanVien(newNhanVien.getID());
                newTaiKhoan.setUsername(txtsdt.getText());
                newTaiKhoan.setPasswords("123");
                new TaiKhoanDao().InsertSQL(newTaiKhoan);
                NV.LoadData();
                NV.LoadSUM();
                dispose();
            }
