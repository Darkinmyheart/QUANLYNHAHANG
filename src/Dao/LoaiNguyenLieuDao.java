...
public class LoaiNguyenLieuDao implements Dao<LoaiNguyenLieu> {
    ...
    public LoaiNguyenLieu getObjectById(String Id) {
        LoaiNguyenLieu loaiNguyenLieuList = new LoaiNguyenLieu();
        try {
            Connection conn = new Conn.ConnectionSQL().getConnection();

            String sql = "SELECT * FROM LoaiNguyenLieu where id like '"+Id+"'";
            PreparedStatement ps = conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                String id = rs.getString("ID");
                ...
                Date ngayTao = rs.getDate("NgayTao");
                Date ngayKetThuc = rs.getDate("NgayKetThuc");
                loaiNguyenLieuList = new LoaiNguyenLieu(id, ten, ngayTao, ngayKetThuc);
